    ...
    if (!isOpen || suggestions.length === 0) {
      ...
      if (e.key === 'Enter' && value.length === 6 && /^[1-9]\d{5}$/.test(value)) {
        ...
        if (exists) {
          ...
          setIsOpen(false);
          inputRef.current?.blur();
          // Call custom handler after validation
          onKeyDown?.(e);
        }
      } else {
        // Dropdown closed, allow custom navigation
        onKeyDown?.(e);
      }
      return;
    }
      ...
      <div className="relative" ref={dropdownRef}>
        <div className="relative">
          ...
          <input
            ...
            className={`block w-full py-2 pl-8 pr-10 bg-white border rounded-lg text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:ring-1 transition ${inputHasErrorUI
